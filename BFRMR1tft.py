...
def StartupScreen():
    ClearScreen()
    DrawHeader()   
    PutLine8x12('INITIALIZING - PLEASE WAIT...',25,80,RED)

def MainScreen():    
    ClearScreen()
    DrawHeader()    
    PutLine8x12('Find Food',25,60,BLACK)
    PutLine8x12('Play',25,80,BLACK)
    PutLine8x12('Explore',25,100,BLACK)
    PutLine8x12('View Data',25,120,BLACK)
    PutLine8x12('Test Moves',25,140,BLACK)
    PutLine8x12('Shutdown',25,160,BLACK)
    PutCh8x12('>',10,60,BLACK) #put > on first line when start screen is drawn

    
def EditMainScreen(PointerPos):
    FillRect(0,60,20,200,bColor)
    if PointerPos is 0:
        PutCh8x12('>',10,60,BLACK)       
    if PointerPos is 1:
        PutCh8x12('>',10,80,BLACK)       
    if PointerPos is 2:
    ...
    if PointerPos is 5:
        PutCh8x12('>',10,160,BLACK)

def FindFoodScreen():    
    ClearScreen()
    DrawHeader()
    PutLine8x12('FINDING FOOD',25,40,BLACK)
    PutLine8x12('Press Button 3 to Stop',40,200,BLACK)


def EditFindFoodScreen(StatusNumber,x,y):
    if StatusNumber is 0:
        FillRect(0,80,320,100,bColor) #clear area of screen
        PutLine8x12("No Target found", 25,80,BLACK)
    if StatusNumber is 1:
        FillRect(0,80,320,100,bColor) #clear area of screen
        PutLine8x12("Target at [" + str(x) + "," + str(y) + "]",25,80,BLACK) 
    if StatusNumber is 2:
        FillRect(0,100,320,120,bColor) #clear area of screen
        PutLine8x12("Left wheel-" + str(x)  + " Right wheel-" + str(y), 25,100,BLACK)
    if StatusNumber is 3:
        FillRect(0,120,320,200,bColor) #clear area of screen
        if x is 0:
            PutLine8x12("No Obstacle Encountered", 25,120,BLACK)
        if x is 1:
            PutLine8x12("Obstacle - Sonar Sensor", 25,120,BLACK)
        if x is 2:
            if y is 0:
                PutLine8x12("Obstacle - Left IR", 25,120,BLACK)
            if y is 1:
                PutLine8x12("Obstacle - Centre IR", 25,120,BLACK)
            if y is 2:
                PutLine8x12("Obstacle - Right IR", 25,120,BLACK)

def ExploreScreen():    
    ClearScreen()
    DrawHeader()
    PutLine8x12('EXPLORING',25,40,BLACK)
    PutLine8x12('Press Button 3 to Stop',40,200,BLACK)
    FillRect(250,195,300,217,GREEN)
    global bColor
    bColor = GREEN
    PutLine8x12("Speed",255,200,BLACK)
    bColor = LIGHTBLUE

def EditExploreScreen(StatusNumber,x,y):
    if StatusNumber is 0:
    ...
    if StatusNumber is 2:
        ...
        StartingY = 190
        for a in range(0, x): #x is speed value 0 -10
           if a < 4:
               FillRect(260,StartingY-10,290,StartingY,GREEN)
               StartingY = StartingY - 12
           elif a >= 4 and a <= 7:
               FillRect(260,StartingY-10,290,StartingY,ORANGE)
               StartingY = StartingY - 12
           else:
               FillRect(260,StartingY-10,290,StartingY,RED)
               StartingY = StartingY - 12 

def ViewDataScreen():
    ClearScreen()
    DrawHeader()    
    PutLine8x12('Left IR',25,60,BLACK)
    PutLine8x12('Centre IR',25,80,BLACK)
    PutLine8x12('Right IR',25,100,BLACK)
    PutLine8x12('Head Pan',25,120,BLACK)
    PutLine8x12('Head Tilt',25,140,BLACK)
    PutLine8x12('Sonar',25,160,BLACK)

def EditViewDataScreen(LeftIR, CentreIR, RightIR, HeadPan, HeadTilt, Sonar):
    FillRect(150,60,320,180,bColor) #clear area of screen
    PutLine8x12(str(LeftIR),150,60,BLACK)
    PutLine8x12(str(CentreIR),150,80,BLACK)
    PutLine8x12(str(RightIR),150,100,BLACK)
    PutLine8x12(str(HeadPan),150,120,BLACK)
    PutLine8x12(str(HeadTilt),150,140,BLACK)
    PutLine8x12(str(Sonar),150,160,BLACK)


def TestMoveScreen():
    ClearScreen()
    DrawHeader()    
    PutLine8x12('Turn 180',25,60,BLACK)
...
def StoppingScreen():
    ClearScreen()
    DrawHeader()
    PutLine8x12('FINISHING CURRENT MOVE',40,80,RED)
    PutLine8x12('PLEASE WAIT...',60,120,RED)
